...
public class SysUserBaseController {
	...
	public SysUserBaseInfo findByName(@RequestBody String username) {
		logger.info("findAllBySearchText", SysUserBaseController.class);
		return sysUserBaseService.findByName(username);
	}
	
	@RequestMapping(path = "/getRoleByUser", method = RequestMethod.POST)
	public List<SysRolesInfo> getRoleByUser(@RequestBody SysUserBaseInfo info) {
		logger.info("findAllBySearchText", SysUserBaseController.class);
		return sysUserBaseService.getRoleByUser(info.getUuid());
	}
